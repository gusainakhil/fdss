...
$has_coach_id    = (bool)$conn->query("SHOW COLUMNS FROM fdds_inventory_unit LIKE 'Coach_id'")->num_rows;
...
if ($has_coach_id && $coach_id > 0)   $join_conds[] = "iu.Coach_id = ?";
...
if ($has_coach_id && $coach_id > 0) { $c_types .= 'i'; $c_params[] = $coach_id; }
...
if ($has_coach_id)    $uq .= ", iu.Coach_id AS coach_id";
...
if ($has_coach_id && $coach_id > 0)   $uq .= " AND iu.Coach_id = ?";
...
if ($has_coach_id && $coach_id > 0) { $u_types .= 'i'; $u_params[] = $coach_id; }
...
while ($row = $unit_result->fetch_assoc()) {
    $units[] = [
        ...
        'coach_id'        => $has_coach_id    ? ($row['coach_id'] !== null ? (int)$row['coach_id'] : null) : null,
    ];
}
$ustmt->close();
...
send_json(200, [
    ...
    'inventory' => [
        ...
        'coach_id'        => ($has_coach_id && $coach_id > 0) ? $coach_id : null,
